...
class MedSegDiff(Module):
    ...
    def forward(self, img, cond_img, *args, **kwargs):
        ...
        b, c, h, w, device, img_size, img_channels, mask_channels = * \
            img.shape, img.device, self.image_size, self.input_img_channels, self.mask_channels

        assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        assert cond_img.shape[
            1] == img_channels, f'your input medical must have {img_channels} channels'
        assert img.shape[
            1] == mask_channels, f'the segmented image must have {mask_channels} channels'
